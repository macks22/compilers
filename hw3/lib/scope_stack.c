...
{   /* Look up a method in the scope stack.
    ...
    return scope_lookup_symbol(class, method_name);
}
...
{   /* Look up an identifier in the scope stack.
    ...
    for (i = stack->size-1; i >= 0; i++) {
        sym = scope_lookup_symbol(stack->scopes[i], name);
        if (sym != NULL) {
            return sym;
        }
    }
...
void
declare_method(ScopeStack *stack, char *name)
{   /* Declare a new method in the current scope. This will fail silently if
     * the current scope is not a class scope.
     */
    assert(stack != NULL);  // sanity check
}

void
declare_attribute(ScopeStack *stack, char *name)
{   /*
     */
    assert(stack != NULL);  // sanity check
}
